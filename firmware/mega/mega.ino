...
// ═══════════════════════════════════════════════════════════
//
// Se llama desde la ISR de Wire cuando el STM32 bridge
// (I2C Master) envía datos al Mega (Slave, addr 0x08).
// Cada byte recibido se pasa al parser igual que Serial1.
...
void setup() {
  ...
  // Inicializar I2C como Slave (reemplaza Serial1 como canal principal)
  // STM32 bridge es Master, Mega es Slave en direccion 0x08
  Wire.begin(I2C_ADDR);
...
void loop() {
  // ── Leer Serial1 (fallback: STM32 bridge aún envía por D0/D1) ──
  // NOTA: I2C se lee en receiveI2C() vía callback. Serial1 se mantiene
  ...
  // ── I2C no se lee aquí: se maneja en receiveI2C() callback ────
